...
### v1.2.8 (2024-12-19)
- **Windows 메모리 최적화 대폭 개선**: 
  - Node.js 메모리 제한을 2GB로 증가
  - 링크 수집기 동시 처리 수를 2개로 제한
  - 가비지 컬렉션 강제 실행으로 메모리 정리
  - Windows 전용 스크립트 추가 (`npm run start:win`)
  - 청크 크기를 1개로 제한하여 안정성 향상

### v1.2.7 (2024-12-19)
- **Windows 브라우저 풀 최적화**: 
  - Windows에서 동시 처리 수를 3개로 제한하여 메모리 부족 문제 해결
  - 브라우저 재시도 로직 추가 (최대 3회)
  - 페이지 재사용으로 메모리 사용량 최소화
  - 청크 크기 조정으로 안정성 향상
  - 브라우저 간 간격 추가로 초기화 안정성 개선

### v1.2.6 (2024-12-19)
- **Windows 메모리 부족 문제 해결**: 
  - 페이징 파일 부족 오류 해결을 위한 메모리 최적화
  - 불필요한 리소스 로딩 차단 (이미지, CSS, 폰트)
  - 메모리 사용량 제한 설정 추가
  - Windows 문제 해결 가이드 문서 추가
  - 브라우저 상태 관리 개선
...
### Windows 페이징 파일 부족 오류

Windows에서 다음과 같은 오류가 발생하는 경우:
```
이 작업을 완료하기 위한 페이징 파일이 너무 작습니다. (0x5AF)
```

**해결 방법:**
1. **가상 메모리 증가**: `WINDOWS_TROUBLESHOOTING.md` 파일 참조
2. **헤드리스 모드 사용**: `npm run start -- --headless`
3. **크롤링 깊이 제한**: `npm run start -- -d 2`

자세한 해결 방법은 [Windows 문제 해결 가이드](WINDOWS_TROUBLESHOOTING.md)를 참조하세요.
